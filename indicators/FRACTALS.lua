...
if (P>0) then
	...
	if c < 3 then
		PrintDbgStr('it_start is '..dump(it))
		PrintDbgStr('Squeeze(it.l,P) (at start) is '..Squeeze(it.l,P))
	end	
	if CandleExist(I,ds) then
		...
		if c < 3 then
			PrintDbgStr('I is '..I)
			PrintDbgStr('it is '..dump(it))
			PrintDbgStr('H_tmp is '..dump(H_tmp))
			PrintDbgStr('Squeeze(it.l,P) is '..Squeeze(it.l,P))
			PrintDbgStr('Ipppp is '..Ipppp)
		end
		c = c + 1
		if it.l >= nP then
			...
			if c < 3 then
				PrintDbgStr('S is '..S)
				PrintDbgStr('val_h is '..val_h)
				PrintDbgStr('val_l is '..val_l)
				PrintDbgStr('L is '..L)
				PrintDbgStr('H is '..H)
			end
